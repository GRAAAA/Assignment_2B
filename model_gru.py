...
def build_gru(seq_len: int = SEQ_LEN, units: int = 64) -> Sequential:
    model = Sequential([
...
def train_gru(dl_data: tuple, epochs: int = 50,
              batch_size: int = 32, verbose: int = 0) -> dict:
    X_tr, X_te, y_tr, y_te, scaler = dl_data

    tf.keras.utils.set_random_seed(42)
    model      = build_gru(seq_len=X_tr.shape[1])
    ...
    return {
        ...
        "scaler":  scaler,
        "history": history,
        "metrics": _compute_metrics(y_true, y_pred, "GRU"),
        "y_test":  y_true,
        "y_pred":  y_pred,
    }


def predict_gru(model, scaler, flow_history: np.ndarray,
...
if __name__ == "__main__":
    ...
    result = train_gru(data["dl_data"], epochs=10, verbose=1)
    m      = result["metrics"]
    print(f"GRU   RMSE={m['rmse']:.4f}  NRMSE={m['nrmse']:.4f}  "
          f"MAE={m['mae']:.4f}  R2={m['r2']:.4f}")
    ts   = data["ts"]
    pred = predict_gru(result["model"], result["scaler"],
